      ...
      {activeTab === 'tasks' ? (
        ...
        <div className="flex-1 flex items-center">
          <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
            {activeTab === 'dashboard' ? (
              <>
                Welcome back, {loading ? <span className="inline-block w-24 h-6 bg-gray-200 animate-pulse rounded-md ml-1"></span> : `${userProfile.username || 'User'}!`}
              </>
            ) : ''}
          </h2>
          {activeTab === 'dashboard' && (
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-bold uppercase">
              {loading ? <div className="w-full h-full bg-green-200/50 animate-pulse rounded-full"></div> : userProfile.username.charAt(0)}
            </div>
